...
object usrActionAnalyze {
  def main(args: Array[String]): Unit = {
    ...
    //过滤后的数据session的明细
    val sessionDetailRDD = getSessionId2DetailRDD(filteridSessionRDD,sessionId2ActionRDD)
    sessionDetailRDD.take(20).foreach(println)
  ...
  def calculateTop10Session(top10CategorySessionClickCount:RDD[(Long,String)],taskid:Long)={
    //分组按品类
    val top10CategorySessionCountsRDD = top10CategorySessionClickCount.groupByKey()
