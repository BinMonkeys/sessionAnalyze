...
import com.qf.sessionanalyze.domain.{AdBlacklist, AdStat, AdUserClickCount}
...
object AdClickRealTimeStatSpark extends Serializable{
  def main(args: Array[String]): Unit = { // 模板代码
   ...
   val ssc = new StreamingContext(conf, Seconds(5))
    ssc.checkpoint("hdfs://192.168.1.130:9000/cp20181219")
    //解析参数
    //指定组名
    val group = "g001"
    val kafkaTopics = ConfigurationManager.getProperty(Constants.KAFKA_TOPICS)
    val topicsSet = Set(kafkaTopics)
    val brokerList = "192.168.1.130:9092,192.168.1.120:9092,192.168.1.110:9092"
