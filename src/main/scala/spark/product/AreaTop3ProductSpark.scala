...
import com.qf.sessionanalyze.util.ParamUtils
...
object AreaTop3ProductSpark {
  def main(args: Array[String]): Unit = {
    ...
    println(taskId)
    if (task == null) System.out.println("没有获取到对应taskid的信息")
    println(task)
  ...
  /**
    ...
    * @return
    */
  private def getCityId2ClickActionRDD(sparkSession: SparkSession, startDate: String, endDate: String) = { // 从user_visit_action基础表中查询用户访问行为数据
  ...
  /**
    ...
    * @return
    */
  private def getCityId2CityInfoRDD(sparkSession: SparkSession) = { // 构建连接MySQl的配置信息
    var url = ""
    var user = ""
    var password = ""
  ...
  /**
    ...
    */
  private def generateTempClickProductBasicTable(sparkSession: SparkSession, cityId2ClickActionRDD: RDD[(Long, Row)], cityId2CityInfoRDD: RDD[(Long, Row)]) = { // 将点击行为和城市信息进行关联：join
  ...
  /**
    ...
    * @param sparkSession
    */
  private def generateTempAreaProductClickCountTable(sparkSession: SparkSession) = { // 按照area和product_id两个字段进行分组
    // 计算出各区域商品的点击次数
  ...
  /**
    ...
    * @param sparkSession
    */
  private def generateTempAreaFullProductClickCountTable(sparkSession: SparkSession) = {
    /**
      * 将之前得到的各区域商品点击次数表(tmp_area_product_click_count)的product_id字段
      * 去关联商品信息表(product_info)的product_id
      * 齐总product_status需要特殊处理：0,1分别代表了自营和第三方商品，放在了一个json里
